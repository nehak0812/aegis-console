...
PLAYBOOKS: dict[str, dict] = {
    ...
    # --- aftermath: the event is historic, but there is still work behind it ----------------------
    "DW-LEAK-180": {
        "owner": "Security operations", "effort": "M",
        "prevents": "The same route being used twice. The listing is months old, so the question is no longer containment but whether the entry point was ever closed.",
        "steps": ["Confirm the incident was investigated and the entry route identified.",
                  "Verify the fix is still in place — configuration drifts back.",
                  "Check whether the same group has since listed anyone in your sector using the same method."],
        "controls": ["NIST CSF 2.0 RC.RP-01", "NIST CSF 2.0 ID.IM-01"],
    },
    "DW-LEAK-OLD": {
        "owner": "Security operations", "effort": "S",
        "prevents": "A historic listing being forgotten rather than closed out.",
        "steps": ["Confirm the incident is closed and the lessons recorded.",
                  "Keep the record: leak-site listings are frequently re-published and re-reported as new."],
        "controls": ["NIST CSF 2.0 ID.IM-01"],
    },
    "DW-LEAK-SUB": {
        "owner": "Security operations", "effort": "M",
        "prevents": "A subsidiary's breach reaching the parent through shared identity, network or supplier paths.",
        "steps": ["Confirm the subsidiary relationship in the GLEIF record before acting on it.",
                  "Establish what the subsidiary shares with the parent — directory, VPN, tooling, suppliers.",
                  "Apply the same containment checks to the shared paths, not just to the subsidiary.",
                  "Decide whether group reporting duties are engaged, with legal."],
        "controls": ["NIST CSF 2.0 GV.SC-01", "NIST CSF 2.0 RS.MA-01"],
    },
    "DW-FORUM-OLD": {
        "owner": "Security operations", "effort": "S",
        "prevents": "An old claim being rediscovered and treated as new.",
        "steps": ["Confirm the earlier assessment and whether credentials were rotated at the time.",
                  "Record the outcome so a repeat claim can be compared rather than re-investigated."],
        "controls": ["NIST CSF 2.0 ID.IM-01"],
    },
    "DW-STEALER-OLD": {
        "owner": "Security operations", "effort": "S",
        "prevents": "Old infostealer exposure staying valid because nothing was rotated.",
        "steps": ["Confirm the affected credentials were rotated when this was first seen.",
                  "Check the accounts have phishing-resistant multi-factor authentication now."],
        "controls": ["NIST CSF 2.0 PR.AA-01"],
    },
    "DW-DDOS-30": {
        "owner": "Network engineering", "effort": "S",
        "prevents": "Assuming a campaign has passed. Target lists are reused, and the same hosts are usually named again.",
        "steps": ["Confirm DDoS protection is active on the named hosts, not just available.",
                  "Review whether the earlier listing produced any measurable impact.",
                  "Keep the hosts on watch for the next campaign window."],
        "controls": ["NIST CSF 2.0 PR.IR-04"],
    },
    "BR-PUBLIC-12M": {
        "owner": "Security operations", "effort": "S",
        "prevents": "Breached credentials from the past year still working, which is the most common way old breaches become new incidents.",
        "steps": ["Confirm resets were forced for accounts matching the breached domain.",
                  "Check the breached credentials against your directory for reuse.",
                  "Confirm multi-factor authentication covers the affected accounts."],
        "controls": ["NIST CSF 2.0 PR.AA-01"],
    },
    "BR-PUBLIC-OLD": {
        "owner": "Security operations", "effort": "S",
        "prevents": "A historic breach being treated as irrelevant when the credentials may never have been rotated.",
        "steps": ["Confirm the accounts involved no longer use the exposed credentials.",
                  "Keep the record for context when the next breach names the same domain."],
        "controls": ["NIST CSF 2.0 PR.AA-01"],
    },
    "DISC-8K-90": {
        "owner": "Legal & compliance", "effort": "L",
        "prevents": "The gap between what was filed and what was actually remediated. A recent 8-K Item 1.05 means the organisation told the market this was material.",
        "steps": ["Read the filing and any amendment, and confirm the current remediation status against it.",
                  "Track whether an amended filing is due as the investigation concludes.",
                  "Confirm the control failures named in the filing are fixed, not just contained.",
                  "Keep security, legal and investor relations working from the same version."],
        "controls": ["NIST CSF 2.0 GV.OC-03", "NIST CSF 2.0 RS.CO-02"],
    },
    "DISC-8K-OLD": {
        "owner": "Legal & compliance", "effort": "M",
        "prevents": "Commitments made in a disclosure quietly lapsing.",
        "steps": ["Confirm the remediation described in the filing was completed.",
                  "Check whether an amended filing was required and made."],
        "controls": ["NIST CSF 2.0 GV.OC-03"],
    },
    "DISC-801": {
        "owner": "Legal & compliance", "effort": "M",
        "prevents": "An incident disclosed under Item 8.01 being handled with less rigour than one filed as material.",
        "steps": ["Read the filing and confirm why it was filed under 8.01 rather than 1.05.",
                  "Confirm the materiality assessment is documented, since that judgement may be revisited.",
                  "Track remediation as you would for any disclosed incident."],
        "controls": ["NIST CSF 2.0 GV.OC-03"],
    },
    "INC-NAMED-30": {
        "owner": "Security operations", "effort": "M",
        "prevents": "Learning about your own incident from the press. Two or more independent publishers naming you is a fact to respond to, whether or not the claim is right.",
        "steps": ["Establish internally whether the reporting is accurate before responding to it.",
                  "If it is, open an incident and follow the normal process.",
                  "If it is not, record why, because the same story will be recycled.",
                  "Agree a position with communications either way."],
        "controls": ["NIST CSF 2.0 RS.CO-02", "NIST CSF 2.0 RS.MA-01"],
    },
    "INC-NAMED-1": {
        "owner": "Security operations", "effort": "S",
        "prevents": "A single report being ignored until others repeat it.",
        "steps": ["Read the source and judge its reliability.",
                  "Confirm internally whether the claim matches anything you know.",
                  "Record the assessment so a second report can be compared quickly."],
        "controls": ["NIST CSF 2.0 RS.CO-02"],
    },
    # --- AI ------------------------------------------------------------------------------------------
    "AI-SERVICE-DNS": {
